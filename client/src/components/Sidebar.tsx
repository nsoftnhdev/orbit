  ...
  return (
    ...
    >
        ...
        {navItems.map((item) => {
          ...
          return (
            <NavLink
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded text-sm transition-all duration-150 border ${isActive ? "bg-pink-50 text-sky-600 border-pink-100" : "text-slate-500 hover:bg-slate-50 border-transparent hover:text-slate-700"}`}
            >
              <item.icon
                className={`size-4.5 shrink-0 ${isActive ? "text-pink-500" : "text-slate-500"}`}
              />
              {item.name}
              {isActive && (
                <span className="ml-auto w-[5px] h-5 rounded-full bg-pink-500" />
              )}
            </NavLink>
          );
        })}
      ...
      <div className="p-4 border-t border-slate-100">
        ...
        <button onClick={logout} className="mt-1 flex items-center gap-2 px-3 py-2 w-full rounded text-sm text-slate-500 hover:bg-pink-50 hover:text-red-500 transition-all duration-150">
